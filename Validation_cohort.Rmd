...
annot_data <- read_excel("Annotations_validation_cohort.xlsx")
```
#preprocessing
```{r}
...
sample_ids_fil <- covplot_prot_mat_fil[covplot_prot_mat_fil$id_count >= 3000,]$sample_id

covplot_prot_mat_fil <- covplot_prot_mat_fil %>%
  filter(sample_id %in% sample_ids_fil)

#also remove samples that had low BCA when I have another sample of the same type from the same patient
remove_lowIDsmultiplesamples <- c("AT385", "AT511")

covplot_prot_mat_fil <- covplot_prot_mat_fil %>%
  filter(!sample_id %in% remove_lowIDsmultiplesamples)

sample_ids_fil <- covplot_prot_mat_fil$sample_id
...
#save
write.csv(prot_mat_fil, "prot_mat_fil_validation.csv")
write.csv(annot_data_fil, "annot_data_fil_validation.csv")
```
...
ggplot(covplot_prot_mat_fil, aes(type_name, id_count)) +
  ...
  geom_bracket(xmin = 1, xmax = 3, y.position = 5950, 
               label = "ns", size = 0.5, label.size = 4) +
  geom_bracket(xmin = 1, xmax = 2, y.position = 5950, 
               label = "", size = 0.5) +
...
```
#determine overlap proteins with discovery cohort
```{r}
limma_all3_cohort1 <- read.csv("limma_IK_IG_ZG_discovery_cohort.csv", row.names = 1)

prots_cohort1 <- limma_all3_cohort1 %>%
...
#bring into the same order as with discovery cohort
...
best_params <- as.list(read.csv("best_params_validation_cohort.csv")[, -1])
