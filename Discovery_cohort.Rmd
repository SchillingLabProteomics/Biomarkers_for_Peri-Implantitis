...
write.csv(prot2gene, "prot2gene.csv")

#load annotation file
annot_data <- read_excel("Annotations_discovery_cohort.xlsx")
```
...
#remove duplicates when measurements had been repeated
...
#remove those samples from prot_mat
prot_mat <- prot_mat[,!(colnames(prot_mat) %in% samples_remove)]
...
#remove the samples that do not fulfill the strict criteria
samples_strict <- annot_data %>%
  filter(keep_strict == "x")

samples_strict <- samples_strict$sample_id

covplot_prot_mat_fil <- covplot_prot_mat %>%
  filter(sample_id %in% samples_strict)

#remove samples with insufficient proteome depth due to techincal reasons
sample_ids_fil <- covplot_prot_mat_fil[covplot_prot_mat_fil$id_count >= 2400,]$sample_id

covplot_prot_mat_fil <- covplot_prot_mat_fil %>%
  filter(sample_id %in% sample_ids_fil)

ggplot(covplot_prot_mat_fil, aes(x = reorder(sample_id, id_count), y = id_count)) +
...
#save
write.csv(prot_mat_fil, "prot_mat_fil_discovery.csv")
write.csv(annot_data_fil, "annot_data_fil_discovery.csv")
```
...
ggplot(covplot_prot_mat_fil, aes(type_name, id_count)) +
  ...
  geom_bracket(xmin = 1, xmax = 3, y.position = 4950, 
               label = "ns", size = 0.5, label.size = 4) +
  geom_bracket(xmin = 1, xmax = 2, y.position = 4950, 
               label = "", size = 0.5) +
  labs(x = "", y = "Protein IDs") +
  scale_y_continuous(limits = c(2430, 5000),
...
ggplot(temp_IK, aes(x = rank_perc, y = log10(median_raw))) +
  ...
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  xlim(0,1) +
  scale_y_continuous(limits = c(2.7,7), breaks = c(3,5,7)) +
...
```
#Global proteome pearson correlation map
```{r}
#data frame with proteins as columns and samples as rows
corr_mat <- t(prot_mat_fil_sparsred)

#calculate pearson correlation
corr_matrix <- cor(corr_mat, use = "pairwise.complete.obs", method = "pearson")


col_map_colors <- colorRamp2(c(-1, -0.6, -0.2, 0, 0.2, 0.6, 1),
                             c("#053061", "#2166ac", "#92c5de", "white", "#f4a582", "#b2182b", "#67001f"))
...
col_dendrogram <- column_dend(draw(heatmap))
col_labels <- labels(col_dendrogram)

corr_matrix_order <- corr_matrix[col_labels, col_labels]

#get bounds of global clusters
col_dendrogram[[2]][[2]][[2]][[2]][[2]][[2]][[2]][[2]]

for (i in 1:3011){
  if (col_labels[i] == "Q9NRW4"){
    print(i)
  }
}

global_cluster1 <- colnames(corr_matrix_order)[1:361]
global_cluster2 <- colnames(corr_matrix_order)[615:1137]
global_cluster3 <- colnames(corr_matrix_order)[1236:1416]
global_cluster4 <- colnames(corr_matrix_order)[1469:1650]
global_cluster5 <- colnames(corr_matrix_order)[1879:1978]
global_cluster6 <- colnames(corr_matrix_order)[2058:3011]
```
#GO enrichment of global clusters
```{r}
global_ego1 <- enrichGO(gene      = sub(";.*", "", global_cluster1),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
global_ego2 <- enrichGO(gene      = sub(";.*", "", global_cluster2),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
global_ego3 <- enrichGO(gene      = sub(";.*", "", global_cluster3),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
global_ego4 <- enrichGO(gene      = sub(";.*", "", global_cluster4),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
global_ego5 <- enrichGO(gene = sub(";.*", "", global_cluster5),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
global_ego6 <- enrichGO(gene = sub(";.*", "", global_cluster6),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
#plot with labeled  proteins
label_IKIG <- c("MMP9", "PRTN3", "S100A8", "DEFA3", "BPI", "SLPI", "CTSS", "ZG16B", "FDCSP", "BPIFB2", "PDLIM7")

limma_all3_IKIG <- limma_all3 %>%
  filter(Contrast == "IKvIG")

ggplot(data = limma_all3_IKIG,
       mapping = aes(x = logFC,
                     y = -log10(adj.P.Val),
  ...
  labs(x = "Log2 (Peri-implantits/Healthy implant)", y = "-Log10 adj. p-value") + 
  theme_manuscript +
  theme(strip.text = element_blank(), strip.background = element_blank(),
        legend.position = "none") +
  scale_y_continuous(limits = c(0,8.1), breaks = c(0, 2, 4, 6)) +
  scale_x_continuous(limits = c(-2.7,2), breaks = c(-2, -1, 0, 1, 2)) +
  scale_color_manual(values = c("IGvZG+up" = "#5d7034",
                                ...
                                "IKvZG+down" = "#7da4b7",
                                "IKvZG+none" = "gray40")) +
geom_label_repel(aes(label = ifelse(Genes %in% label_IKIG, as.character(Genes), '')),
                 min.segment.length = 0, max.overlaps = Inf,
                 nudge_x = ifelse(limma_all3_IKIG$logFC > 0, 0.2, -0),
                 nudge_y = ifelse(-log10(limma_all3_IKIG$adj.P.Val) < 4, 2.1, 0.2), 
                 box.padding = 0.3, direction = "both", size = 3.5)


label_IKZG <- c("PRTN3", "S100A9", "DEFA3", "SLPI", "CTSS", "LAMC2", "LAMA3", "BPIFB2", "HBB", "HBD", "HBA2", "UBE2V2")

limma_all3_IKZG <- limma_all3 %>%
  filter(Contrast == "IKvZG")

ggplot(data = limma_all3_IKZG,
       mapping = aes(x = logFC,
                     y = -log10(adj.P.Val),
                     color = plot_col_05_contrast)) + 
  geom_point(size = 1) + 
  ...
  theme(strip.text = element_blank(), strip.background = element_blank(),
        legend.position = "none") +
  scale_y_continuous(limits = c(0,8.1), breaks = c(0, 2, 4, 6)) +
  scale_x_continuous(limits = c(-3.5,4.1), breaks = c(-2, 0, 2, 4)) +
  scale_color_manual(values = c("IGvZG+up" = "#5d7034",
                                "IGvZG+down" = "#7da4b7",
                                "IGvZG+none" = "gray40",
                                "IKvIG+up" = "#c44826",
                                ...
                                "IKvZG+down" = "#7da4b7",
                                "IKvZG+none" = "gray40")) +
geom_label_repel(aes(label = ifelse(Genes %in% label_IKZG, as.character(Genes), '')),
                 min.segment.length = 0, max.overlaps = Inf,
                 nudge_x = ifelse(limma_all3_IKZG$logFC > 0, 0., -0),
                 nudge_y = ifelse(-log10(limma_all3_IKZG$adj.P.Val) < 4, 1.7, 0.5), 
                 box.padding = 0.1, direction = "both", size = 3.5)

label_IGZG <- c("HBB", "HBA2", "HBD")

limma_all3_IGZG <- limma_all3 %>%
  filter(Contrast == "IGvZG")

ggplot(data = limma_all3_IGZG,
       mapping = aes(x = logFC,
                     y = -log10(adj.P.Val),
                     color = plot_col_05_contrast)) + 
  geom_point(size = 1) + 
  ...
  scale_color_manual(values = c("IGvZG+up" = "#5d7034",
                                ...
                                "IKvZG+down" = "#7da4b7",
                                "IKvZG+none" = "gray40")) +
geom_label_repel(aes(label = ifelse(Genes %in% label_IGZG, as.character(Genes), '')),
                 min.segment.length = 0, max.overlaps = Inf,
                 nudge_x = ifelse(limma_all3_IGZG$logFC > 0, 0.2, -1),
                 nudge_y = +0.7, box.padding = 0.3, direction = "both", size = 3.5)

```
#hierarchical clustering of dysregulated proteins adj.p-value <= 0.05 - z-norm, then median
```{r}
prots_dys <- c(limma_all3$Protein.Group[limma_all3$Differentially_expressed])
prots_dys <- unique(prots_dys)

prot_mat_dys <- prot_mat_fil_sparsred[prots_dys,]

#do z-score normalization first, then calculate the medians

#z-score normalization
prot_mat_dys_z <- t(scale(t(prot_mat_dys), center = TRUE, scale = TRUE))

#calculate the median of z-score normalized values per condition
med_IK <- apply(prot_mat_dys_z[,sample_ids_IK, drop = FALSE], 1, median, na.rm = T)
med_IG <- apply(prot_mat_dys_z[,sample_ids_IG, drop = FALSE], 1, median, na.rm = T)
med_ZG <- apply(prot_mat_dys_z[,sample_ids_ZG, drop = FALSE], 1, median, na.rm = T)

plot_df <- as.data.frame(prot_mat_dys_z) %>%
  mutate(IK = med_IK) %>%
  mutate(IG = med_IG) %>%
  mutate(ZG = med_ZG)

plot_df <- plot_df[,c(101:103)] %>%
  rownames_to_column("Protein.Group")

plot_df$Protein.Group_short <- sub(";.*", "", plot_df$Protein.Group)

plot_df <- left_join(plot_df, prot2gene)

custom_colors <- colorRamp2(c(-1, -0.5, -0.2, 0, 0.2, 0.5, 1),
                             c("#134688",
                               "#74add1",  
                               "#abd9e9",
                               "white",
...
#make the heatmap
heatmap_obj <- Heatmap(
  t(as.matrix(plot_df[,c(2:4)])),
  #column_order = c("IK", "IG", "ZG"),
  show_row_names = FALSE,
  show_column_names = FALSE,
...
cluster_up <- plot_df[plot_df$IK > 0,]$Protein.Group
#361 proteins

cluster_down <- plot_df[plot_df$IK < 0,]$Protein.Group
#556 proteins
```
#GO over-representation analysis with clusters - adj. p-value 0.05 cutoff
```{r}
ego1_up <- enrichGO(gene      = sub(";.*", "", cluster_up),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
ego2_down <- enrichGO(gene      = sub(";.*", "", cluster_down),
                 OrgDb         = org.Hs.eg.db,
                 keyType = "UNIPROT",
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
#plot as dot plot
ego1_up_res <- ego1_up@result %>%
  arrange(p.adjust) %>%
  head(15)

ego1_up_res <- ego1_up_res %>%
  mutate("Gene ratio" = as.numeric(gsub("/.*", "", GeneRatio))/as.numeric(gsub(".*/", "", GeneRatio))) %>%
  arrange(`Gene ratio`) %>%
  mutate(rank = row_number())

ggplot(ego1_up_res, aes(x = `Gene ratio`, y = reorder(Description, rank))) + 
  geom_point(aes(color = p.adjust, size = Count)) +
  #geom_vline(xintercept = 0, linetype = "solid", color = "gray20", linewidth = 0.5) +
  labs(y = "", color = "Adj. p-value") +
  theme_manuscript +
  theme(legend.position = "right",
        panel.grid.major.y = element_line(color = "grey80", linewidth = 0.5),
        panel.grid.major.x = element_blank(),
        legend.title = element_text(size = 10, color = "black")) +
  scale_color_gradient(low = "#a50026", high = "#fdae61")


ego2_down_res <- ego2_down@result %>%
  arrange(p.adjust) %>%
  head(15)

ego2_down_res <- ego2_down_res %>%
  mutate("Gene ratio" = as.numeric(gsub("/.*", "", GeneRatio))/as.numeric(gsub(".*/", "", GeneRatio))) %>%
  arrange(`Gene ratio`) %>%
  mutate(rank = row_number())

ggplot(ego2_down_res, aes(x = `Gene ratio`, y = reorder(Description, rank))) + 
  geom_point(aes(color = p.adjust, size = Count)) +
  #geom_vline(xintercept = 0, linetype = "solid", color = "gray20", linewidth = 0.5) +
  labs(y = "", color = "Adj. p-value") +
  theme_manuscript +
  theme(legend.position = "right",
        ...
        panel.grid.major.x = element_blank(),
        legend.title = element_text(size = 10, color = "black")) +
  scale_color_gradient(low = "#134688", high = "#abd9e9",labels = scales::scientific)
```



#determine overlap proteins with validation cohort
```{r}
limma_all3_cohort2 <- read.csv("limma_IK_IG_ZG_validation_cohort.csv", row.names = 1)

#overlap of all identified proteins (after sparsred)
all_prots_cohort1 <- limma_all3 %>%
  filter(Contrast == "IKvIG") 
all_prots_cohort1 <- all_prots_cohort1$Protein.Group
#3011 proteins input for limma - cohort 1

all_prots_cohort2 <- limma_all3_cohort2 %>%
  filter(Contrast == "IKvIG")
all_prots_cohort2 <- all_prots_cohort2$Protein.Group
#4021 proteins input for limma - cohort 2

#2954 proteins overlap
all_prots_both <- intersect(all_prots_cohort1, all_prots_cohort2)

#overlap of significant proteins
all_sig_prots_cohort1 <- limma_all3 %>%
  filter(Contrast == "IKvIG") %>%
  filter(Differentially_expressed == TRUE)
all_sig_prots_cohort1 <- all_sig_prots_cohort1$Protein.Group
#757 sig. proteins for limma - cohort 1

all_sig_prots_cohort2 <- limma_all3_cohort2 %>%
  filter(Contrast == "IKvIG") %>%
  filter(Differentially_expressed == TRUE)
all_sig_prots_cohort2 <- all_sig_prots_cohort2$Protein.Group
#1482 sig. proteins for limma - cohort 2

#484 proteins overlap
all_sig_prots_both <- intersect(all_sig_prots_cohort1, all_sig_prots_cohort2)


prots_cohort1 <- limma_all3 %>%
  filter(Contrast == "IKvIG") %>%
  filter(Differentially_expressed == TRUE) %>% 
  filter(logFC >= 0.8 | logFC <= -0.8)
prots_cohort1 <- prots_cohort1$Protein.Group

prots_cohort2 <- limma_all3_cohort2 %>%
  filter(Contrast == "IKvIG") %>%
  filter(Differentially_expressed == TRUE) %>% 
  filter(logFC >= 0.8 | logFC <= -0.8)
prots_cohort2 <- prots_cohort2$Protein.Group

overlap_08prots <- intersect(prots_cohort1, prots_cohort2)
#80 proteins

write.csv(overlap_08prots, "overlap_08prots.csv")

for_corr <- data.frame(Protein.Group = all_prots_both)

for_corr <- left_join(for_corr, limma_all3[limma_all3$Contrast == "IKvIG",c(1,7)])
colnames(for_corr)[2] <- "logFC_Discovery"

for_corr <- left_join(for_corr, limma_all3_cohort2[limma_all3_cohort2$Contrast == "IKvIG",c(1,7)])
colnames(for_corr)[3] <- "logFC_Validation"

#add column to mark the sig overlap proteins
for_corr <- for_corr %>%
  mutate(overlapsig = ifelse(Protein.Group %in% all_sig_prots_both, "yes", "no"))

#add column to mark the 80 overlap proteins
for_corr <- for_corr %>%
  mutate(overlap80 = ifelse(Protein.Group %in% overlap_08prots, "yes", "no"))

#calculate pearson correlation
corr_all <- cor(for_corr$logFC_Discovery, for_corr$logFC_Validation, method = 'pearson')

corr_sig <- cor(for_corr$logFC_Discovery[for_corr$overlapsig == "yes"],
               for_corr$logFC_Validation[for_corr$overlapsig == "yes"],
               method = 'pearson')

corr_80 <- cor(for_corr$logFC_Discovery[for_corr$overlap80 == "yes"],
               for_corr$logFC_Validation[for_corr$overlap80 == "yes"],
               method = 'pearson')

#make column for plotting col
for_corr <- for_corr %>%
  mutate(col = case_when(overlap80 == "yes" ~ "c",
                         overlap80 == "no" & overlapsig == "yes" ~ "b",
                         overlapsig == "no" ~ "a"))

ggplot(for_corr %>%
         arrange(col), aes(x = logFC_Discovery, y = logFC_Validation, color = col)) +
  ...
  ylim(-2.7, 2) +
  xlim(-2.7, 2) +
  scale_color_manual(values = c("c" = "#006d2c", "b" = "#74c476", "a" = "grey70"),
...
best_params <- as.list(read.csv("best_params_discovery_cohort.csv")[, -1])
